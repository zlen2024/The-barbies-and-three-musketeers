...
const Layout = ({ children, isFixed = false, isDark = false }) => {
  ...
  return (
    <div className={`flex flex-col h-screen ${isDark ? 'bg-gray-900 text-gray-100' : 'bg-slate-50'}`}>
      ...
      {/* Main Content */}
      <main className={`flex-1 flex flex-col min-h-0 w-full ${!isFixed ? 'max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 overflow-y-auto' : 'overflow-hidden'}`}>
        {children}
      </main>

      {/* Footer */}
      <footer className={`${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'} border-t flex-none`}>
